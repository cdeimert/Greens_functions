...
\note{
    \begin{itemize}
    \item
        This is intended as a small (dense) introduction to Green's functions for electrical engineers.
    \item
...
\begin{frame}[fragile]
    ...
    %    \\
    %    \uncover<2->{\inprod{\delta(x - x')}{H(x,x'')}}
    %    &\uncover<2->{= \inprod{G(x,x')}{\delta(x - x'')}}
    %    \\
    %    \uncover<3->{\int_a^b \delta(x - x') H^*(x,x'') \d x}
    %    &\uncover<3->{= \int_a^b G(x,x') \delta(x - x'') \d x}
    %    \\
    %    \uncover<4->{H^*(x',x'')} 
    %    &\uncover<4->{= G(x'',x')}
    %\end{align*}
...
\note{
    ...
    \item
        ...
        For one thing, it means that if $ G(x,x') $ obeys the boundary conditions with respect to $ x $, then it automatically obeys the \emph{adjoint} boundary conditions with respect to $ x' $. 
...
\item
    We've spent this entire course learning different ways to solve the Helmholtz equation, which is just a Fourier-transformed version of the wave equation.
\item
    ...
    So we would like to find a Green's functions approach to solving the scalar wave equation.
\end{itemize}
}
...
\item
    The first step is to find the adjoint operators and adjoint boundary conditions.
\item
...
\begin{frame}[fragile]
    ...
    \item
        Separation of variables (3D problems)
    \item
