...
\begin{frame}[fragile]
    ...
    Green's function problem:
    \begin{align*}
        \frac{\d^2 G(x,x')}{\d x^2} + \omega_0^2 G(x,x') &= \delta(x - x')
    \end{align*}
...
\note{
    ...
    \item
        Question: what should the boundary conditions be for $ G(x,x') $?
    \end{itemize}
}
...
\begin{frame}[fragile]
    ...
    \begin{align*}
        G(x,x') \left[ \frac{\d^2 u(x)}{\d x^2} + \omega_0^2 u(x) \right] &= G(x,x') f(x)
    \end{align*}

    \begin{align*}
        u(x) \left[ \frac{\d^2 G(x,x')}{\d x^2} + \omega_0^2 G(x,x') \right] &= u(x) \delta(x - x')
    \end{align*}
...
\note{
    ...
    \item
        First, we multiply the original equation by $ G(x,x') $ and the Green's functions equation with by $ u(x) $.
    \end{itemize}
}
...
\begin{frame}[fragile]
    ...
    \begin{align*}
        \int_a^b \Big[ u(x) \frac{\d^2 G(x,x')}{\d x^2} &- G(x,x') \frac{\d^2 u(x)}{\d x^2} \Big] \d x 
        \\&= \int_a^b \left[ u(x) \delta(x - x') - G(x,x') f(x) \right] \d x
    \end{align*}
...
\begin{itemize}
\item
    Next, we subtract and integrate over $ [a,b] $.
\item
    The integral over $ u(x) \delta(x - x') $ becomes just $ u(x') $.
\item
    The left hand side can be dealt with using integration by parts.
\end{itemize}
}
...
\begin{frame}[fragile]
    ...
    \begin{align*}
        ...
        \\
        &\quad + \Big[ u(x) \frac{\d G(x,x')}{\d x} - G(x,x') \frac{\d u(x)}{\d x} \Big]_{x=a}^b
    \end{align*}
