...
\begin{itemize}
\item
    Don't have time to do much here, but we'll sketch out the basic foundations and look at the simple example of the wave equation.
\end{itemize}
}

\begin{frame}[fragile]
    \frametitle{Green's second identity}

    \begin{align*}
        \int_{V} \left( \psi \nabla^2 \phi - \phi \nabla^2 \psi \right) \d v &= \oint_{\partial V} \left( \phi \vect{\nabla} \psi - \psi \vect{\nabla} \phi \right) \cdot \d \vect{s}
    \end{align*}

    \pause
    Looks kind of like
    \begin{align*}
        \rinprod{\L \psi}{\phi} - \rinprod{\psi}{\L \phi} &= \int_{\partial V} \vect{J}(\phi, \psi) \cdot \d \vect{s}
    \end{align*}
    with the ``reaction'' (pseudo) inner product
    \begin{align*}
        \rinprod{\psi}{\phi} &= \int_V \psi \phi \d v
    \end{align*}
...
\begin{itemize}
\item
    A key theorem for 3D Green's functions is an integral theorem known as Green's second identity.
\item
    We won't do it here, but it's not too difficult to derive using the vector product rule
    \begin{align*}
        \vect{\nabla} \cdot (f \vect{\nabla} g) &= (\vect{\nabla} f) \cdot (\vect{\nabla} g) + f \nabla^2 g
    \end{align*}
    and the divergence theorem.
\item
    If we treat the volume integral as an inner product and $ \nabla^2 $ as our operator $ \L $, this looks a lot like an expression we had from 1 dimension.
    We have a term with the operator, a term with the adjoint operator (in this case $ \L^* = \L $), and a leftover conjunct term which depends only on the boundaries.
\end{itemize}
}
...
\begin{itemize}
\item
    Note: the reaction inner product is a pseudo inner product and not a true inner product.
    A true inner product would be, e.g.,
    \begin{align*}
        \inprod{\psi}{\phi} = \int_V \psi \phi^* \d v
    \end{align*}
    Without the complex conjugate, we have some weird properties.
    For example, the ``length'' of a vector $ \left\| \phi \right\|^2 = \rinprod{\phi}{\phi} $ can be negative or complex.
\item
    However, the reaction (pseudo) inner product still plays an important role in electromagnetism, particularly with regards to reciprocity.
    See, e.g., Chew's paper ``A New Look at Reciprocity and Energy Conservation Theorems in Electromagnetics.''
\end{itemize}
}
...
\begin{frame}[fragile]
    ...
    Original problem:
    \begin{align*}
        \left( \nabla^2 + \frac{\partial^2}{\partial t^2} \right) u(\vect{r}, t) &= f(\vect{r}, t); \quad \B[u] = \alpha
    \end{align*}

    Green's problem:
    \begin{align*}
        \left( \nabla^2 + \frac{\partial^2}{\partial t^2} \right) G(\vect{r}, t; \vect{r}', t') &= \delta^3(\vect{r} - \vect{r}') \delta(t - t'); \quad \B[G] = 0
    \end{align*}

    Adjoint Green's problem:
    \begin{align*}
        \left( \nabla^2 + \frac{\partial^2}{\partial t^2} \right) H(\vect{r}, t; \vect{r}', t') &= \delta^3(\vect{r} - \vect{r}') \delta(t - t'); \quad \B^*[H] = 0
    \end{align*}
...
\begin{itemize}
\item
    Can follow a similar derivation as the 1D case, but with the reaction inner product. I should probably just state the key results: expression for $ u $ in terms of $ H $, expression for $ G $ in terms of $ H $, and final expression for $ u $.
\item
    Note: I'm doing the wave equation because it's interesting (and explains the causality requirement on the Helmholtz Green's function!) but a lot of the EM books only do the Helmholtz equation.
\end{itemize}
}
