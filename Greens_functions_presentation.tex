...
\begin{frame}[fragile]
    ...
    \begin{IEEEeqnarray*}{rCl}
        \int_a^b \left( \frac{\d^2 u(x)}{\d x^2} \right) v^*(x) \d x &=& \int_a^b u(x) \left( \frac{\d^2 v^*(x)}{\d x^2} \right) \d x +
        \\ && + \left[ \frac{\d u}{\d x} v^*(x) - u(x) \frac{\d v^*(x)}{\d x} \right]_a^b
    \end{IEEEeqnarray*}

    So $ \L = \L^* $, $ \B = \B^* $, and
    \begin{align*}
        J(u,v^*) &= \frac{\d u(x)}{\d x} v^*(x) - u(x) \frac{\d v^*(x)}{\d x}
    \end{align*}
...
\item
    ...
    Our solution for the unknown $ V(x) $ includes the unknown $ V(x) $.
\item
    But we could solve this if we could show that $ G(x,a) = G(x,b) = 0 $.
\end{itemize}
}
...
\begin{frame}[fragile]
    ...
    Adjoint Green's problem:
    \begin{align*}
        \frac{\d^2 H(x,x')}{\d x^2} &= \delta(x - x'); \quad \mat{H(a,x') \\ H(b,x')} = \mat{0 \\ 0}
    \end{align*}

    But $ G(x,x') = H^*(x',x) $ so
    \begin{align*}
        \frac{\d^2 G^*(x,x')}{\d x^{\prime 2}} &= \delta(x - x'); \quad \mat{G(x,a) \\ G(x,b)} = \mat{0 \\ 0}
    \end{align*}
...
\item
    More importantly, we see that $ G^*(x,x') $ (and thus $ G(x,x') $) obeys the adjoint boundary conditions with respect to $ x' $.
\item
...
\begin{frame}[fragile]
    ...
    \begin{IEEEeqnarray*}{rClCrCl}
        \text{Given: \quad} \L u(x) = f(x); \quad \B[u(x)] = \alpha
    \end{IEEEeqnarray*}
    \begin{enumerate}
    \item
...
\item
    The first step is to set up the Green's function equation by setting the source to $ \delta(x - x') $ and the boundary conditions to zero.
    Solve this to find the Green's function.
\item
    Next, we find $ \L^* $, $ \B^* $, and $ J(u,v) $ by expanding the inner product $ \inprod{\L u}{v} $ (usually using integration by parts).
\item
    Then, we can write down the solution.
\item
    Unfortunately, this solution will still include unknown boundary conditions of $ u(x) $.
...
\begin{itemize}
\item
    Type of boundary condition is important.
    In the last example, if we used initial conditions instead of boundary conditions, then the final solution would have been in terms of the known initial conditions $ V(a) $ and $ V'(a) $.
\item
    \emph{We can't ignore the adjoint problem}.
    Though we've eliminated the adjoint Green's function $ H(x,x') $, the adjoint Green's function is still a key part of the solution method.
    In particular, we need to find $ \B^* $ and $ J(u,v) $.
\end{itemize}
}
...
\begin{frame}[fragile]
    ...
    \begin{IEEEeqnarray*}{rClCrCl}
        \text{Given: \quad} \L u(\vect{r}) = f(\vect{r}); \quad \B[u(\vect{r})] = \alpha
    \end{IEEEeqnarray*}
    \begin{enumerate}
    \item
        Solve Green's problem:
...
\item
    Mostly, we just replace $ x $ and $ x' $ with $ \vect{r} $ and $ \vect{r}' $.
\item
    One difference is the conjunct term.
    If $ V $ is the region of interest, then we have to integrate the conjunct over the surface of $ V $ (denoted $ \partial V $).
\item
    We still have the same interpretation, though: the non-zero boundary conditions on the surface act like additional sources.
\end{itemize}
}
...
\begin{itemize}
\item
    To find $ \L^* $, $ \B^* $ and $ J(u,v) $ in 3D, we follow a similar process of expanding the inner product. 
    (This time the inner product is a volume integral.)
\item
    Then we do a 3D version of integration by parts to expand the integral.
    If we're dealing with an equation involving the Laplacian $ \nabla^2 $ (often the case), then Green's identity comes in handy.
\end{itemize}
